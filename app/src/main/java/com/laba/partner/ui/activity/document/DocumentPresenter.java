...
public class DocumentPresenter<V extends DocumentIView>
        extends BasePresenter<V> implements DocumentIPresenter<V> {
    ...
    @Override
    public void postUploadSingleDocuments(RequestBody id, MultipartBody.Part document) {
        getCompositeDisposable().add(
                APIClient
                        .getAPIClient()
                        .postUploadSingleDocuments(id, document)
                        .subscribeOn(Schedulers.computation())
                        .observeOn(AndroidSchedulers.mainThread())
                        .subscribe(getMvpView()::onDocumentSuccess, getMvpView()::onError));
    }
